...
クライアントに配る記入用テンプレートも同じディレクトリにあります（Import画面からもDL可）。

- `samples/wbs_template.xlsx` … WBS記入用。「このカテゴリのゴール」列つきで、取り込むと
  カテゴリ（親タスク）の説明として保持され、Task管理のカテゴリ行に表示されます
- `samples/issue_list_template.xlsx` … 課題記入用。取り込むと課題分析プレビューに進みます
