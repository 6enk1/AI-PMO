...
### 課題記入用テンプレート（クライアント配布用）

「課題を挙げてください」と頼んでも、整った表は返ってこない。返ってきた形のまま取り込める
ようにしてあるが、**先に配れるテンプレート**も用意した。Import画面の
「課題リストのテンプレート（.xlsx）をダウンロード」、または `GET /api/imports/template`。

- シートは2枚。「課題リスト」（記入欄）と「記入のしかた」（記入例つきの説明）
- **必須は「課題・気になっていること」の1列だけ。** 残りは空欄でよい
- 記入するセルは水色、記入例はグレー斜体で1行だけ。数式は使っていない
- 重要度は `高 / 中 / 低` のプルダウン、期限は日付の入力規則つき
- 列を増やしても、順番を入れ替えても取り込める（列名から自動判定するため）

記入された **重要度・期限は課題分析プレビューへそのまま引き継がれる**（AIの推定より
記入値を優先し、「記入された重要度「高」を使用」と根拠に残す）。関連するタスク・工程の
列は、関連タスク候補の絞り込みに使う。

テンプレートの実体は `samples/issue_list_template.xlsx`（`python samples/generate_issue_template.py` で再生成）。
