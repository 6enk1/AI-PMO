...
ファイルの中身を見て、WBS（タスク一覧）か課題リストかを判定し、**関係のない設定を隠す**。
判定はヘッダー名だけでなく、実際に値が入っているかまで見る（列名だけ揃った空の表を誤判定しない）。

| 判定 | 表示される設定 |
| --- | --- |
| WBS（タスク一覧） | 取り込み先・既存Taskとの突き合わせ（課題列の設定は非表示） |
| 課題リスト | 取り込み先のみ。**Taskは作らず**、課題分析プレビューへ進む |
| タスク＋課題 | 両方 |
| 判別できず | 「WBSですか？課題リストですか？」と一度だけ確認する |

課題列は列名（課題／懸念／気になっていること…）で拾えなければ、**中身の長さから自由記述列を探す**。
先方の独自の列名でも取りこぼさないようにしてある。判定根拠は画面に表示され、手動で変更もできる。

### 記入用テンプレート（クライアント配布用）

出てきた資料をそのまま取り込めるようにはしてあるが、**先に配れるテンプレート**も用意した。
Import画面の「WBSのテンプレート」「課題リストのテンプレート」、または
`GET /api/imports/template?kind=wbs|issues`。どちらも記入例1〜2行と「記入のしかた」シート付き、
数式なし。列を増やしても順番を変えても取り込める。

#### WBSテンプレート（`wbs_template.xlsx`）

| No | 大カテゴリ | このカテゴリのゴール | タスク（必須） | タスクの詳細 | 担当者 | 開始予定日 | 期限 | 状態 | 先行タスク | 備考 |

**必須は「タスク」の1列だけ。** 状態は `未着手 / 進行中 / 完了 / 保留` のプルダウン、日付は入力規則つき。

**進捗率（%）の列は置いていない。** 計画を出してもらう時点では埋まらないし、状態で足りる。
取り込み時に「完了」は100%として扱い、それ以外は推測しない。進捗率を運用していない
プロジェクト（途中経過の%が1件も無い）では、**進捗ベースの指摘と加点を止める**
（`due_soon_low_progress` と Risk の `progress_gap`）。全Taskを一律に「遅れている」と
言わないための扱いで、%を1件でも入れた時点から通常どおり効く。

肝は **「このカテゴリのゴール」**。担当・期限・進捗だけを集めても「何のためにやっているのか」は
残らないので、カテゴリごとに達成したい状態を1行書いてもらう（そのカテゴリの1行目だけでよい）。
取り込むとカテゴリ（親タスク）の説明として保持され、**Task管理のカテゴリ行に 🎯 付きで表示**される。
タスク側の「詳細」は上書きしない。同じファイルのゴールを書き直して再取り込みすれば更新される。

ゴールの列名は `ゴール / 目的 / 目標 / 達成条件 / 完了条件 / アウトカム / goal …` を自動で拾う。

#### 課題リストテンプレート（`issue_list_template.xlsx`）

| No | 課題・気になっていること（必須） | 関連するタスク・工程 | 重要度 | 期限 | 記入者 | 備考 |

- **必須は「課題・気になっていること」の1列だけ。** 残りは空欄でよい
- 文章のままでよく、1セルに複数書いても取り込み時に分割する
- 重要度は `高 / 中 / 低` のプルダウン、期限は日付の入力規則つき

記入された **重要度・期限は課題分析プレビューへそのまま引き継がれる**（AIの推定より
記入値を優先し、「記入された重要度「高」を使用」と根拠に残す）。関連するタスク・工程の
列は、関連タスク候補の絞り込みに使う。

テンプレートの実体は `samples/wbs_template.xlsx` / `samples/issue_list_template.xlsx`
（`python samples/generate_templates.py` で再生成）。
...
| GET | `/api/imports/template?kind=wbs\|issues` | クライアント記入用テンプレートの配布 |
